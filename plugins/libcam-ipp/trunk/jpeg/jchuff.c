...
encode_mcu_gather_intellib (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ci = cinfo->MCU_membership[blkn];
    compptr = cinfo->cur_comp_info[ci];
    htest_one_block_intellib(cinfo, MCU_data[blkn][0], entropy->saved.last_dc_val[ci],
        entropy->dc_count_ptrs[compptr->dc_tbl_no],
        entropy->ac_count_ptrs[compptr->ac_tbl_no]);
    entropy->saved.last_dc_val[ci] = MCU_data[blkn][0][0];
  }
...
#else
      jpeg_gen_optimal_table_intellib(cinfo, *htblptr, entropy->dc_count_ptrs[dctbl]);
#endif
      did_dc[dctbl] = TRUE;
    }
    if (! did_ac[actbl]) {
...
#else
      jpeg_gen_optimal_table_intellib(cinfo, *htblptr, entropy->ac_count_ptrs[actbl]);
#endif
      did_ac[actbl] = TRUE;
    }
  }
